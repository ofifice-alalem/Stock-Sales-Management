    ...
    <table class="w-full">
        ...
        <tbody class="divide-y divide-white/5">
            ...
            <tr class="hover:bg-white/5 transition-colors">
                ...
                <td class="px-6 py-4">
                    <div class="flex gap-2">
                        <a href="{{ route('marketer.invoices.show', $invoice->id) }}" class="text-blue-600 hover:text-blue-700">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                            </svg>
                        </a>
                        <a href="{{ route('marketer.invoices.edit', $invoice->id) }}" class="text-yellow-600 hover:text-yellow-700">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="md:hidden space-y-4">
    ...
    <div class="stat-card rounded-2xl p-4 border border-white/5 shadow-xl">
        ...
        <div class="flex gap-2">
            <a href="{{ route('marketer.invoices.show', $invoice->id) }}" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-all text-center text-sm">
                عرض
            </a>
            <a href="{{ route('marketer.invoices.edit', $invoice->id) }}" class="flex-1 bg-yellow-600 hover:bg-yellow-700 text-white px-4 py-2 rounded-lg transition-all text-center text-sm">
                تعديل
            </a>
            <form action="{{ route('marketer.invoices.destroy', $invoice->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من الحذف؟')" class="flex-1">
                @csrf
                @method('DELETE')
                <button type="submit" class="w-full bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg transition-all text-sm">
                    حذف
                </button>
            </form>
        </div>
